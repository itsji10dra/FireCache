...
extension FireManagerTests {
    ...
    func testFetchJSONWithJSONManager() {
        
        let expectation = self.expectation(description: "Wait for downloading JSON")
        
        jsonManager.fetch(with: jsonURL) { (json, url, error) in
            XCTAssertNotNil(json)
            XCTAssertNil(error)
            XCTAssertEqual(jsonURL, url)
            
            let isCached = self.jsonManager.cache.containsObject(forKey: jsonURL.absoluteString)
    ...
    func testFetchImageWithJSONManager() {
        
        let expectation = self.expectation(description: "Wait for downloading image")
        
        jsonManager.fetch(with: imageURL) { (image, url, error) in
            XCTAssertNil(image)
            XCTAssertNotNil(error)
            XCTAssertEqual(imageURL, url)
            
            let isCached = self.jsonManager.cache.containsObject(forKey: imageURL.absoluteString)
    ...
    func testFetchStringWithJSONManager() {
        
        let expectation = self.expectation(description: "Wait for downloading string")
        
        jsonManager.fetch(with: stringURL) { (string, url, error) in
            XCTAssertNil(string)
            XCTAssertNotNil(error)
            XCTAssertEqual(stringURL, url)
            
            let isCached = self.jsonManager.cache.containsObject(forKey: stringURL.absoluteString)
            XCTAssertFalse(isCached)
            expectation.fulfill()
        }
